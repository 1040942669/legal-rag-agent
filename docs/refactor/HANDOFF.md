...
- M0 实现和发布前验收记录已完成，状态为 `ready_for_review`；包含验收记录的新 PR head 尚待精确 SHA CI，因此还不是 `ready_for_release` 或 `released`。
...
1. 提交并 push 当前验收报告、STATE 和 HANDOFF。
2. 等待 PR #2 对包含这些文档的精确 head 重新运行 CI；不得复用 `b3a2b46...` 的旧绿灯。
3. 复核 base/head、完整 diff、CI 和仓库规则，随后将 draft PR 标为 ready 并正常合并，不使用 admin bypass。
...
| 阻塞 | 无实现阻塞；状态门禁为最终文档 head CI 尚未运行 |
| 下一条可执行动作 | 提交/push 本交接和验收报告，等待精确新 head CI |
