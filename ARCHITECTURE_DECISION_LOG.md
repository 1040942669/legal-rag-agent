...
历史记录中“证据支持”或 verifier 通过应按当时启发式口径理解：引用编号存在、免责声明和有限词面规则不等于语义蕴含或法律正确性。当前改造主线在 M0 先公开这一限制；M1 候选已拆分结构、行为与语义状态，详见第 17 条，但尚不能据此宣称真实法律质量提高。
...
检索固定为 BM25 + article（四组 Hit@5 完全一致），30 条子集上对比 4 个 backend，DeepSeek-V3 做 judge。本地只保留 qwen2.5:7b（q4_K_M），其余走 SiliconFlow API。结果: DeepSeek-V3 judge 在该固定子集上给出的 faithfulness 普遍 ≥0.94；这不能单独归因于 prompt 或引用检查，也不能据此排除幻觉。Qwen3.5-4B 推理型 legacy verifier pass 最高（0.633）但延迟 141s 不可用于交互；本地 4bit 7B 在该子集上未观察到明显劣势，但它与 API Instruct 模型并非同一权重，因此不能把结果单独解释为严格的量化无损实验。
...
- DeepSeek-V3 同时作为 judge 和被评模型之一，该行存在 self-preference 风险；规则 verifier 只是确定性启发式而非无偏真值，仍需人工抽查校准。
...
## 17. M1 将结构、行为和语义状态分开，不用一个总通过率代替真值

### 问题 / 触发点

旧 verifier 把引用编号、免责声明、宽泛拒答词和有限词面启发式汇总为一个 `passed`。这会产生三类误导：免责声明可能被当成拒答；真实存在的来源编号可能被误读为已经支持 claim；无需拒答的普通 case 会自动抬高旧拒答均值。Retrieval-only 还曾把检索片段拼成“回答”，使 answer-only 指标看起来可用。

### 最终决策

- 生成适配器使用四种明确回答模式，并为旧纯文本保留低可验证性的兼容路径。
- 结构层分别记录 schema、证据目录、引用 ID、可见引用与 claim 对齐、快照/权限范围。
- 行为层分别记录免责声明、回答模式、明确拒答、资料不足和澄清；受限模式只接受有界模板。
- 语义层使用 `supported / unsupported / uncertain / not_checked`。当前词面启发式不允许自动产生 `supported`。
- 被 verifier 拒绝的生成草稿与重新验证后的最终交付响应分开记录，质量指标只统计最终交付。
- schema v2 以 `value + unavailable_reason` 表示不可用值，并显式报告应答、应拒答、应澄清、服务失败和 Judge 三态分母。
- Retrieval-only 不运行 answer verifier 或 Judge；旧 CSV 的 `-1` 只作为兼容哨兵。

### 兼容性与回滚

旧文本生成器和旧 CSV 字段继续可用，但不能重新启用“免责声明等于拒答”或“没有发现词面问题等于语义支持”的语义。若业务兼容需要回切旧文本适配器，仍保留新的安全终态、显式 N/A 和分母规则。历史报告不覆写，也不把 schema v1 与 v2 总通过率直接比较。
