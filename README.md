...
annotated `v0.4.0` Tag 对象 `1aa41823030681e17b7da70c27b50463d7d997b1` 精确 peeled 到上述 release target；GitHub Release 已于 `2026-09-25T00:22:49Z` 发布，非 draft、非 prerelease、附件 0。软件发布事实已经成立；当前状态是 `released_receipt_pending`，独立机器回执已写入 `docs/refactor/receipts/M3.json`，仍需经过自己的 PR、CI 和 master 复验后才能把 M3 标为完整 `released`。
